...
function formatReport(results, args, pattern) {
  const lines = [];
...
function formatVerbose(results, args, pattern) {
  const lines = [];
  const baseReport = formatReport(results, args, pattern);
...
function formatJson(results, args) {
  const payload = {
...
module.exports = { formatReport, formatVerbose, formatJson };
